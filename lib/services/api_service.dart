...
import 'web_http_stub.dart'
    if (dart.library.html) 'web_http.dart';
...
class ApiService {
  ...
  Future<Map<String, String>> _headers({bool auth = false}) async {
    ...
    // ngrok adds an interstitial "browser warning" page for non-browser clients.
    // This header bypasses it so API calls work correctly from the mobile app.
  ...
  Future<ApiResponse> get(String endpoint,
    ...
    try {
      if (kIsWeb) {
        ...
        if (webRes == null) return ApiResponse.error('Web HTTP unavailable', code: 0);
  ...
  Future<ApiResponse> post(String endpoint,
    ...
    try {
      if (kIsWeb) {
        final webRes = await webPost(url, body, await _headers(auth: requiresAuth));
        if (webRes == null) return ApiResponse.error('Web HTTP unavailable', code: 0);
        final r = http.Response(webRes.body, webRes.statusCode);
        if (_needsRefresh(r.statusCode) && !isRetry && requiresAuth) {
          final ok = await TokenManager.instance.forceRefresh();
          if (ok) return post(endpoint, body: body, requiresAuth: true, isRetry: true);
          _expiredSession();
        }
        return _parse(r);
      }
  ...
  Future<ApiResponse> patch(String endpoint,
    ...
    try {
      if (kIsWeb) {
        final webRes = await webPatch(url, body, await _headers(auth: requiresAuth));
        if (webRes == null) return ApiResponse.error('Web HTTP unavailable', code: 0);
        final r = http.Response(webRes.body, webRes.statusCode);
        if (_needsRefresh(r.statusCode) && !isRetry && requiresAuth) {
          final ok = await TokenManager.instance.forceRefresh();
          if (ok) return patch(endpoint, body: body, requiresAuth: true, isRetry: true);
          _expiredSession();
        }
        return _parse(r);
      }
  ...
  ApiResponse _parse(http.Response r) {
    ...
    // ── Detect HTML error pages (Django debug 404/500) ──────────────────────
    // This must happen BEFORE jsonDecode to prevent "Invalid argument" crash
    ...
    if (r.body.trim().isEmpty) {
      return r.statusCode >= 200 && r.statusCode < 300
          ...
          : ApiResponse.error('Empty response (\${r.statusCode}).', code: r.statusCode);
    }
  ...
  ApiResponse _netErr(Object e, String url) {
    ...
    if (kIsWeb && s.contains('Failed to fetch'))
      return ApiResponse.error(
          ...
          'CORS_ALLOW_ALL_ORIGINS = True', code: 0);
    if (s.contains('SocketException') || s.contains('Connection refused') ||
        s.contains('Failed host lookup'))
      return ApiResponse.error(
          'Cannot reach server.\nRun: python manage.py runserver\n'
          'URL: ${ApiConstants.baseUrl}', code: 0);
...
class ApiResponse {
  ...
  const ApiResponse._({required this.isSuccess, required this.statusCode,
