...
/// Shows: header, stats (Total/Approved/Rejected),
/// Approved section + Rejected section (matching Image 1 exactly),
/// "New Request" card at bottom → Home tab.
...
class _RequestsDashboardScreenState extends State<RequestsDashboardScreen> {
  ...
  Widget _body() {
    ...
    return ListView(
      ...
      children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: 4),
                  Icon(Icons.arrow_forward_ios_rounded, size: 12, color: Color(0xFF2C5F4F)),
                ]),
              ),
            ),
          ],
        ...
        Row(children: [
          _StatBox(value: '${d?.total ?? 0}',
              label: 'Total', valueColor: const Color(0xFF2C3E3C)),
          const SizedBox(width: 12),
          _StatBox(value: '${d?.approved ?? 0}',
              label: 'Approved', valueColor: const Color(0xFF27AE60),
              bgColor: const Color(0xFFF0FBF4)),
          const SizedBox(width: 12),
          _StatBox(value: '${d?.rejected ?? 0}',
              label: 'Rejected', valueColor: _red,
  ...
  Widget _sectionHeader({
    ...
    required bool showViewAll,
      ...
      Row(children: [
        ...
        const Spacer(),
        // View All — always shown so user can see all filter tabs
        GestureDetector(
          onTap: () => Get.to(() => const MyRequestsScreen(),
              transition: Transition.cupertino),
          child: const Text('View All',
              style: TextStyle(fontSize: 14, color: Color(0xFF2C5F4F),
  ...
  Future<void> _goToQR(ApprovedRequest req) async {
    // Build a minimal RequestModel for QRScannerScreen
    ...
    final result = await Get.to(() => QRScannerScreen(request: rm),
        transition: Transition.cupertino);
    if (result == true) _load(); // refresh after successful scan
  }

  // ── New Request card ───────────────────────────────────────────────────────
  Widget _newRequestCard() => GestureDetector(
    onTap: () {
      try { Get.find<BottomNavController>().changeTab(0); } catch (_) { Get.back(); }
    },
    child: Container(
...
class _StatBox extends StatelessWidget {
  ...
  const _StatBox({
    ...
    required this.valueColor, this.bgColor = Colors.white,
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// Approved Request Card  (matches Image 1 exactly)
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            _badge(Icons.check_circle_outline_rounded, 'Approved',
                _green, const Color(0xFFF0FBF4)),
            const SizedBox(width: 8),
            Text(req.ref, style: const TextStyle(
          ...
          Text(req.serviceName, style: const TextStyle(
              fontSize: 17, fontWeight: FontWeight.bold,
              color: Color(0xFF2C3E3C))),
          const SizedBox(height: 6),
          Row(children: [
            if (req.sector.isNotEmpty) ...[
              const Icon(Icons.home_outlined, size: 14, color: Color(0xFF9E9E9E)),
              const SizedBox(width: 4),
              Text(req.sector, style: const TextStyle(
                  fontSize: 12, color: Color(0xFF9E9E9E))),
              const SizedBox(width: 12),
            ],
            if (req.approvedAt.isNotEmpty) ...[
              ...
              const SizedBox(width: 4),
              Text(req.approvedAt, style: const TextStyle(
                  fontSize: 12, color: Color(0xFF9E9E9E))),
            ],
          ]),
        ]),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          const Spacer(),
          OutlinedButton(
            onPressed: onScanQR, // → QRScannerScreen
...
// ─────────────────────────────────────────────────────────────────────────────
// Rejected Request Card  (matches Image 1 exactly)
// ─────────────────────────────────────────────────────────────────────────────
class _RejectedCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        _badge(Icons.cancel_outlined, 'Rejected',
            _red, const Color(0xFFFFF5F5)),
        const SizedBox(width: 8),
        Text(req.ref, style: const TextStyle(
            fontSize: 12, color: Color(0xFF9E9E9E),
            fontWeight: FontWeight.w600)),
      ...
      if (req.rejectionReason.isNotEmpty) ...[
        ...
        // Divider line before reason (matching Image 1)
        const Divider(height: 1, color: Color(0xFFEEEEEE)),
        const SizedBox(height: 8),
        Text('Reason: ${req.rejectionReason}',
            style: const TextStyle(fontSize: 13,
                color: Color(0xFF5A5A5A), height: 1.4),
