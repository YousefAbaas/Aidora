...
class SignUpScreen extends StatefulWidget {
  /// 'volunteer' → calls registerVolunteer endpoint; otherwise registerRefugee
  final String? role;
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  bool _validate() {
    ...
    setState(() {
      ...
      _emailError = !_emailCtrl.text.contains('@') ? 'Enter a valid email' : null;
      _passError  = _passwordCtrl.text.length < 6  ? 'Min 6 characters'  : null;
      if (_passwordCtrl.text != _confirmCtrl.text) {
        _passError = 'Passwords do not match';
      }
    });
    if (_nameError != null || _emailError != null || _passError != null) ok = false;
    if (!_acceptTerms) {
      Get.snackbar('Consent Required', 'Please accept the data consent to continue.',
          backgroundColor: Colors.orange[400], colorText: Colors.white,
          snackPosition: SnackPosition.TOP, borderRadius: 12,
  ...
  Future<void> _register() async {
    ...
    if (result.isSuccess) {
      Get.off(() => OtpVerificationScreen(email: result.email, role: widget.role),
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text('Fill in your information to register as a volunteer',
                      style: TextStyle(fontSize: 13, color: Colors.grey[600])),
                  const SizedBox(height: 28),

                  // Full name
                  _label('Full Name'),
                  _field(controller: _nameCtrl, hint: 'e.g. Sara Ahmed',
                      icon: Icons.person_outline, error: _nameError),
                  const SizedBox(height: 16),

                  // Phone
                  _label('Phone Number'),
                  _field(controller: _phoneCtrl, hint: '+963 XXX XXX',
                  ...
                  // Email
                  _label('Email'),
                  _field(controller: _emailCtrl, hint: 'name@email.com',
                      icon: Icons.email_outlined,
                      keyboardType: TextInputType.emailAddress, error: _emailError),
                  const SizedBox(height: 16),
                  ...
                  _field(controller: _confirmCtrl, hint: 'Repeat password',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: error != null ? Colors.red[300]! : Colors.transparent),
        ),
        child: TextField(
          controller: controller, obscureText: obscure,
          keyboardType: keyboardType,
          decoration: InputDecoration(
            hintText: hint, hintStyle: TextStyle(color: Colors.grey[400], fontSize: 15),
      ...
      if (error != null) Padding(
        ...
        child: Text(error, style: TextStyle(fontSize: 12, color: Colors.red[700])),
      ),
    ]);
  }
}
