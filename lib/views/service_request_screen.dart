...
///     success → {message, request_id} → back with success snackbar
///     profile incomplete → {detail: "You must complete your profile first."}
///       → should not happen (caller checks), but shown as error
/// ─────────────────────────────────────────────────────────────────────────────
...
class _ServiceRequestScreenState extends State<ServiceRequestScreen> {
  ...
  Future<void> _fetchServiceInfo() async {
    setState(() { _loadingInfo = true; _infoError = null; });
    final r = await _svc.fetchServiceRequestInfo(widget.orgId, widget.serviceId);
    if (!mounted) return;
    if (r.isSuccess) {
      setState(() {
      ...
      setState(() { _infoError = r.errorMessage; _loadingInfo = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loadingInfo
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF2C5F4F)))
          : _infoError != null
              ? _errorWidget()
              : _form(context),
    );
  ...
  Widget _errorWidget() => Center(
    child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text(_infoError!, textAlign: TextAlign.center,
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Provided by ${widget.orgName}',
              style: const TextStyle(
                  fontSize: 12, color: Color(0xFF2C5F4F),
