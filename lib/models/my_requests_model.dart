...
class MyRequestsModel {
  ...
  factory MyRequestsModel.fromJson(Map<String, dynamic> j) {
    ...
    final approved = (j['Approved'] ?? countsMap['Approved'] ?? 0 as num).toInt();
    final rejected = (j['Rejected'] ?? countsMap['Rejected'] ?? 0 as num).toInt();
...
class ApprovedRequest {
  ...
  const ApprovedRequest({
    required this.id, required this.ref, required this.serviceName,
    required this.status, required this.sector, required this.approvedAt,
  });
...
class RejectedRequest {
  ...
  const RejectedRequest({
    required this.id, required this.ref, required this.serviceName,
    required this.rejectionReason, required this.status,
  });
