...
/// Shows [text] normally, or replaces with '••••' when privacy mode is on.
class PrivacyText extends StatelessWidget {
  final String text;
  final TextStyle? style;
  final TextAlign textAlign;
  const PrivacyText(this.text, {super.key, this.style, this.textAlign = TextAlign.start});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final hidden = SettingsController.to.privacyMode;
      return Text(hidden ? _mask(text) : text, style: style, textAlign: textAlign);
    });
  }
  ...
  String _mask(String s) {
    ...
    if (m != null && m.group(2)!.isNotEmpty) {
      return '${m.group(1)}${'•' * m.group(2)!.length.clamp(4, 8)}';
    }
    return '•' * s.length.clamp(4, 8);
  }
}
      ...
      return Stack(children: [
        ...
        Positioned.fill(
          child: Container(
            ...
            child: const Center(
              child: Icon(Icons.visibility_off_outlined, color: Colors.white60, size: 26),
            ),
          ),
        ),
      ]);
