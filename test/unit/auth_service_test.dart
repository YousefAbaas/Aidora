...
AuthService _svc({
  ...
  ApiResponse? forgot,
    ...
    AuthService.testInstance(FakeApiService(posts: {
...
void main() {
  group('AuthService.login()', () {
    ...
    test('volunteer login returns role volunteer', () async {
      final r = await _svc(login: ok(Map.from(loginVolunteerJson)))
          .login(email: 'v@v.com', password: 'pass');
      expect(r.isSuccess, isTrue);
  ...
  group('AuthService.registerVolunteer()', () {
    test('success same contract as registerRefugee', () async {
      final r = await _svc(registerVolunteer: ok(Map.from(registerSuccessJson)))
          .registerVolunteer(
