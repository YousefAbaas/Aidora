...
void main() {
  ...
  group('ApiConstants — URL construction', () {
    test(
        'organizationCards correct',
        () => expect(ApiConstants.organizationCards,
            equals('/api/organizations/cards/')));
    test(
        'organizationFilter encodes type',
        () => expect(ApiConstants.organizationFilter('Education'),
            equals('/api/organizations/filter/Education/')));
    test(
        'organizationDetail builds with id',
        () => expect(ApiConstants.organizationDetail(1),
            equals('/api/organizations/1/')));
    test(
        'orgServiceRequest builds correctly',
        () => expect(ApiConstants.orgServiceRequest(5, 10),
            equals('/api/requests/org/5/services/10/request/')));
    test(
        'tokenRefresh is correct',
        () => expect(
            ApiConstants.tokenRefresh, equals('/api/auth/token/refresh/')));
    test('login endpoint',
        () => expect(ApiConstants.login, equals('/api/auth/login/')));
    test(
        'registerRefugee endpoint',
        () => expect(ApiConstants.registerRefugee,
            equals('/api/auth/register/refugee/')));
    test(
        'completeProfile is PATCH endpoint',
        () => expect(ApiConstants.completeProfile,
            equals('/api/auth/refugees/complete-profile/')));
    test(
        'refugeeProfile endpoint',
        () => expect(
            ApiConstants.refugeeProfile, equals('/api/auth/profile/refugee/')));
    test(
        'uploadProfileImage is PATCH endpoint',
        () => expect(ApiConstants.uploadProfileImage,
            equals('/api/auth/profile/upload-image/')));
    test(
        'myRequests endpoint',
        () => expect(
            ApiConstants.myRequests, equals('/api/requests/my-requests/')));
    test(
        'requestList with filter query',
  ...
  group('OrganizationDetailModel', () {
    ...
    test('handles missing impact images', () {
      final m = OrganizationDetailModel.fromJson({
        'name': 'T',
        'title': 'T',
        'logo': 'http://127.0.0.1:8000/l.png',
        'about': 'A',
        'services': [],
        'target_groups': [],
      });
  ...
  group('Request bodies', () {
    ...
    test('service request body fields', () {
      ...
      expect(
          b.keys, containsAll(['family_members', 'description', 'location']));
    });
